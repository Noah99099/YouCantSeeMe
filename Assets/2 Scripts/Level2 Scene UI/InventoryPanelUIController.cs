...
public class InventoryPanelUIController : MonoBehaviour
{
    ...
    [Header("預覽物品腳本")] // *** 新增分類，方便管理功能腳本 ***
    public ModelPreviewPanelUIController modelPreviewController; // *** 新增引用 ***
    ...
    {
        // *** 移除：modelPreviewPanel.SetActive(true) 和 Debug.Log，因為將轉移到 ModelPreviewPanelUIController 的 ShowModelPreview 中 ***
        EventSystem.current.SetSelectedGameObject(null); //清除所有UI焦點避免出問題
        ...
        // 1. 將 ModelPreview map 推入棧
        InputStackManager.Instance.PushMap(InputActionMaps._ModelPreview);

        // 2. *** 呼叫 ModelPreviewPanelUIController 來執行顯示和激活面板的工作 ***
